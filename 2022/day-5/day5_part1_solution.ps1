...
# Solution for Part 1 of Advent of Code 2022 Day 5
# Created 5 Dec 2022 by Jack Truckenmiller in collaboration with https://github.com/bgalliford

# Part 1 Goal: Find the top value of nine stacks after reorganizing the stacks using a set of provided
...
foreach ($instruction in $instructions) {
    ...
    # Loop through for the number of values to be popped off the origin stack and push them onto the destination stack
    for ($i =0; $i -lt $numberToMove; $i++) {
        $stacks[($moveToStack - 1)].Push($stacks[($moveFromStack - 1)].Pop())
    }
}
